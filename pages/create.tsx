  ...
  const handleCreateListing = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (listingType?.value === "directlisting") {
        await createDirectListing({
            assetContractAddress: process.env.NEXT_PUBLIC_COLLECTION_CONTRACT!, // ! is to imply that its defently there else  we might get unassigned also collection contract bcoz nft are referred to collection address
            tokenId: selectedNft.metadata.id,  // we get the token id
            currencyContractAddress: NATIVE_TOKEN_ADDRESS, //currency is goerli 
            listingDurationInSeconds: 60 * 60 * 24 * 7, // 1 week
            quantity: 1, // makes it unique.
            buyoutPricePerToken: price.value,
            startTimestamp: new Date()
        }, {
            onSuccess(data, variables, context) {
                console.log('Success',data, variables, context);
                router.push('/');  // will redirect to home screen
            },
        ...
        {selectedNft && (
          <form onSubmit={handleCreateListing}>
            <div className="flex flex-col p-10">
              ...
              <button className=" bg-blue-500 text-white font-semibold tracking-wide rounded-lg p-3 text-center mt-8 "
                type="submit">
                Create Listing
              </button>
            </div>
